...
- 2026-08-15：实现完成（分支 `07-search-provider-release-gate`，worktree `.tmp/issue07-search-provider-worktree`，conda 环境 `agent`）。要点：
  ...
  - **发布门总装**：`RELEASE_GATE_PYTHON_TESTS` 纳入金标路由（`test_golden_intent_routes.py`，Issue 03）与降级语义（`test_teaching_gate.py`，Issue 02）作为门禁组成部分执行；单命令 `python scripts/release_gate.py --real-probes --qwen-authenticity` 任一硬门失败非零退出，报告只含脱敏字段。
  - **回归**：`tests/closeout`、`tests/web_search`、`tests/security`、`tests/learning/test_teaching_gate.py`、`tests/chat/test_golden_intent_routes.py` 等通过（79 项定向 + 发布门测试集 288 passed / 1 skipped）；全量测试套件结果见实现提交说明。ruff 全绿；mypy 相对 main 基线无新增错误（worktree 环境下 104 项为 main 基线既有问题，baseline worktree 同输出，与本次改动无关）。
