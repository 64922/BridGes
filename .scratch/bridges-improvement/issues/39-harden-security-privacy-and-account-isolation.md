...
- [x] 会话 Cookie 根据实际传输环境设置正确的 HttpOnly、SameSite、路径、生命周期和安全属性；登出、撤销、过期及无效 Cookie 会被清除且不会形成登录重定向循环。
- [x] 所有改变账户、数据、权限、插件、提醒和资产状态的浏览器请求具有有效 CSRF 防护和来源校验，跨站表单与脚本请求不能成功。
- [x] 切换账户会停止旧流式响应、播放、上传和后台订阅，清空页面缓存、搜索建议、选择状态与本地临时数据，并要求敏感操作重新认证。
- [x] 所有上传、归档解包、SKILL 安装、备份恢复和资产路径都拒绝路径穿越、绝对路径、符号链接逃逸、名称混淆和越界覆盖。
- [x] 日志、遥测、错误响应和审计不包含密码、百炼 Key、SMTP 授权码、会话令牌、加密密钥、完整私人正文或未经必要处理的模型请求。
- [x] Qwen 仅接收当前任务授权的最小会话、画像和材料切片；DuckDuckGo 只接收本地去身份关键词，每次披露有可审计类别和授权快照。
- [x] 每个后台任务、调度器作业、媒体任务、索引任务和缓存条目都绑定稳定账户与对象范围，账户撤权或删除后不能继续产出可见结果。
- [x] SKILL 和 MCP 无法读取秘密、未授权文件、其他账户对象或未声明网络域名，也无法通过子进程、环境继承、链接、重定向或提示注入提升权限。
- [x] 直接对象标识、下载地址、对话标识、项目标识、任务标识和审计标识均执行服务端账户授权，猜测标识不能读取或推断其他账户状态。
- [x] 所有发现的高危和中危用例均有回归测试、修复证据和剩余风险说明；发布门对未关闭高风险项失败闭锁。

## Verification

- [x] 建立 Cookie 固定、失效会话、CSRF、登录重定向、退出失败和账户切换并发攻击测试。
- [x] 使用两个以上账户并发执行聊天、搜索、上传、媒体、提醒、画像反馈、导出和插件调用，验证页面、缓存、任务和结果不串号。
- [x] 使用恶意文件名、归档、符号链接、备份和 SKILL/MCP 夹具执行路径逃逸与覆盖测试。
- [x] 向模型、搜索、日志和审计注入秘密金丝雀，自动扫描所有出站请求及落盘记录，确认未发生泄露。
- [x] 执行 MCP 网络、文件、命令、子进程和敏感操作越权测试，并验证权限撤回即时生效。
- [x] 形成可复现安全报告，列出攻击步骤、预期拒绝、实际证据、修复回归及仍接受的明确风险。
...
- 2026-08-06：全部 AC 与验证项完成并提交（安全报告见
  `docs/security/issue39-security-report.md`；新增安全回归测试位于
  `tests/security/` 与 `apps/web/e2e/issue39-security.spec.ts`）。
  核心修复：新增 CSRF 来源校验中间件（`api/csrf.py` +
  `BRIDGES_ALLOWED_ORIGINS`）；媒体对象/分镜/沙箱运行账户隔离；
  MCP 符号链接与重定向闭锁 + 受限工作目录；日志/审计金丝雀脱敏与
  生产禁 cassette 录制；前端账户切换清空本地暂存、停止流式/朗读；
  后台任务删除后无产出回归。
