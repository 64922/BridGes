...
- [ ] 版本化评测包具有固定数据清单、授权与许可证记录、任务定义、运行矩阵、模型与 SKILL 版本、随机种子、评分量表和预期产物 Schema。
- [ ] 数据覆盖多轮画像记录—调用—回答—反馈—修正闭环，并量化画像正确性、越界写入、后续个性化收益、自然度和跨轮稳定性。
- [ ] 人味评测覆盖科普文案、课程讲稿、科研汇报和论文写作，测量模板腔、机翻感、AI 味、任务适配度和事实不变性。
- [ ] 科学评测测量事实准确、引用支持、校准、主题完整一致、幻觉和证据冲突处理，并单列高风险失败案例。
- [ ] 教学评测覆盖先备诊断、步骤规划、适当测验、知识库不足时强制联网及学习结果，不以对话长度代替学习增益。
- [ ] 生涯规划和陪伴评测验证事实/假设区分、风险信号边界、敏感推断抑制和有分寸表达，不把情绪识别当心理诊断。
- [ ] 多模态评测覆盖 ASR 转写、TTS 可懂度、图片/视频提示遵循、资产可用性、替代说明、失败恢复和固定模型合同。
- [ ] 对比至少包含完整 BridGes、基础 Qwen 和合法开源参考方法，并包含移除画像切片、移除 bridges-humanizer、移除证据检索等可解释消融。
- [ ] 盲评隐藏系统身份并随机化顺序，记录评审一致性和分歧；自动裁判不能成为唯一结论来源。
- [ ] 报告展示样本量、点估计、区间、显著性或不确定性、成本、时延、失败率、逐切片结果和代表性失败案例。
- [ ] 任一固定快照、数据或评分逻辑变化都会生成新的运行锁和报告版本，旧结果仍可追溯且不会被静默覆盖。

## Verification

- [ ] 在干净环境中连续运行两次固定评测，比较运行锁、样本集合、结构化产物和允许随机范围内的指标差异。
- [ ] 对故意注入的事实错误、画像越界、模板腔、风险误判、教学跳步和多模态失败验证指标能检出回归。
- [ ] 检查所有外部参考的许可证、来源和使用范围，确认未复制无许可证项目的文本、结构或示例。
- [ ] 由不知道系统身份的评审者完成配对盲评，并对低一致性项目执行复核而非强行合并。
- [ ] 从报告中的失败案例追溯到固定输入、运行锁、工具记录和输出，证明可一键重放同一案例。
- [ ] 将发布阈值接入最终发布门，完整系统未达到最低事实、安全和稳定性阈值时阻止发行。
