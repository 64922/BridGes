...
- [x] 已登录且核心模型探测可用的用户可以创建对话、发送中文消息并看到真实模型正文逐步呈现。
- [x] 每个对话、用户消息、助手消息、生成状态、耗时与固定模型运行锁持久化并绑定稳定账户 ID。
- [x] 重启完整运行时后，用户可以重新打开同一对话并看到顺序、正文和状态一致的历史消息。
- [x] 发送期间有明确中文生成状态和停止入口；连接断开会保留已接收正文并显示可重试错误，不重复插入用户消息。
- [x] 重试创建新的助手尝试并保留审计关系，不静默改写历史；失败不会注册为成功回答。
- [x] 无 Key、Key 无效、核心能力不可用、网络失败、供应商拒绝和限流分别显示可操作中文提示。
- [x] 第二账户不能读取、订阅、重试或推测第一账户的对话和消息；切换账户时旧流立即与界面隔离。
- [x] 生产路径不调用 Stub，不把工作流节点 ID、调试字段、系统提示或凭据输出给用户。
- [x] 聊天页面完整覆盖 loading、empty/new conversation、streaming、error、permission/unauthenticated 和恢复后正常状态，不出现占位页。
- [x] 电脑端仅用键盘可聚焦输入框、发送、停止、重试并返回；流式更新不会抢夺焦点或让屏幕阅读器逐 token 重复朗读。
