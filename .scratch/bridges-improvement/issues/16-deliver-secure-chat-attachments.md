...
- [x] 两种对话模式的同一“+”菜单都能选择受支持文件或常见图片，并保持当前会话与模式。
- [x] 服务端通过内容嗅探校验类型和大小，拒绝可执行文件、伪造扩展名、路径穿越、空文件和超限文件。
- [x] 每个对象以稳定账户 ID 归属，并记录内容哈希、原始显示名、安全媒体类型、大小、创建时间和关联消息。
- [x] 上传中的附件可取消；失败项显示中文原因并可重试；重试不会创建重复对象或重复消息。
- [x] 上传成功后消息显示文件名、类型、大小和处理状态，刷新与重启后仍可见。
- [x] 下载必须经过授权接口；响应和页面不泄露宿主绝对路径、对象密钥或其他账户元数据。
- [x] 用户可从消息中删除附件；确认后同步解除引用并按归属和引用计数安全清理对象及派生数据。
- [x] 越权查看、下载、删除或把其他账户对象绑定到当前消息均被拒绝，且不泄露资源是否存在。
- [x] 上传区域与附件卡片具有中文 loading、empty、error、permission 和 recovery 状态，按钮均连接真实后端。
- [x] 桌面键盘可完成打开文件选择、取消、重试、下载和删除，状态变化通过可访问文本呈现。
...
本次交付新增聊天附件的版本化迁移、内容嗅探与宏文件拒绝、账户/会话授权、幂等上传与按上传标识取消、消息事务绑定、授权下载、确认删除和对象清理；日常陪伴与学习模式共用 Composer 的真实上传状态机。验证：附件集成测试 3 passed，聊天/存储/身份/OpenAPI 回归 121 passed，全量 Python 测试 1196 passed、2 skipped；前端 typecheck、附件相关 ruff/mypy 通过；干净桌面 E2E 21 passed，修复既有菜单测试钩子后单测 1 passed。
