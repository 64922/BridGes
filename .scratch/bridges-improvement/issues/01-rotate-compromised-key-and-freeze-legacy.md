...
- [ ] 密钥持有人已在供应商侧吊销或轮换疑似泄露的 Key，并以不包含密钥正文的记录确认完成时间和操作者。
      （供应商控制台操作为人工动作，代码代理不能替代；确认记录补记位置见封存清单。）
- [x] 旧 Key 已从当前运行配置、开发脚本、测试夹具、日志样例和文档示例中移除；任何检查输出都不得回显秘密值。
- [x] 代码库具备秘密扫描或等价检查，能够阻止百炼 Key、SMTP 授权码、密码和会话令牌被提交。
      （`tests/security/test_secret_scan.py` 防回归扫描，输出只报告位置不回显秘密值。）
- [x] 旧数据库已生成只读封存清单，记录文件名、大小、哈希和封存时间；没有真实账户、上传材料或有效画像被声明为待迁移数据。
      （`.scratch/bridges-improvement/sealed-legacy-science-companion.md`。）
- [x] BridGes 初始化不会读取或原位修改旧数据库，也不会修改旧 Wayfinder `.scratch` 与根目录旧 `tickets.md`。
- [x] 安全文档使用中文，明确说明事件处置、恢复方式和旧数据不可自动迁移的边界。
      （`docs/security/incident-001-legacy-qwen-key.md`。）
...
模块审查（2026-08-02）发现初版仅勾选验收标准而无真实产物，已补齐：
秘密扫描防回归测试（`tests/security/test_secret_scan.py`）、旧数据库只读
封存清单（`.scratch/bridges-improvement/sealed-legacy-science-companion.md`，
含三个文件的大小与 SHA-256）、中文安全文档
（`docs/security/incident-001-legacy-qwen-key.md`）。供应商侧吊销仍为
人工动作，AC1 保持未勾选，等待密钥持有人确认。
