...
- [ ] 切片编译按任务、模式、适用范围、授权、敏感度、冻结/撤回状态和有效期筛选，只返回最小必要记录。
- [ ] 未确认候选、已撤回、已冻结禁止调用或范围不匹配的记录不会进入模型上下文。
- [ ] 用户可在发送前关闭本轮画像使用；关闭后模型请求、审计和上下文说明均不含画像切片。
- [ ] 每条使用画像的回答提供可展开中文说明，展示类别、用途、来源记录链接和本次使用时间，不暴露隐藏提示或原始思维链。
- [ ] 回答只披露材料类别及可访问引用，不把完整私人材料、完整画像或秘密凭据复制到审计日志。
- [ ] 用户可标记“回答不合适”并给出偏好反馈，也可标记“画像有误”并直接修正、冻结或撤回对应记录。
- [ ] 画像修正后的下一轮回答使用新版本；历史回答保留当时切片版本，能够回放修正前后差异。
- [ ] 学习模式使用相关目标、知识状态和学习证据调整教学；日常模式只使用必要偏好与情境，不强制教学结构。
- [ ] 多账户并发和后台调用均按稳定账户 ID 编译切片，不共享缓存或审计内容。
- [ ] 上下文说明与反馈流具有中文 loading、empty、error、permission 和 recovery 状态，失败不丢失用户反馈。
