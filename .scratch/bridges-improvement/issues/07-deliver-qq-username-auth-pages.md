...
- [x] 注册页是完整 BridGes 桌面页面，显示原创 Logo、用户名、QQ 邮箱、密码、密码显隐、提交按钮和前往登录入口。
- [x] 仅接受本地部分全为数字且域名严格为 `qq.com` 的邮箱；用户名规范化后大小写不敏感唯一；QQ 邮箱全局唯一。
- [x] 登录页只提供一个“用户名或 QQ 邮箱”标识字段和密码字段，可分别用当前用户名或 QQ 邮箱登录同一账户。
- [x] QQ 邮箱注册时不发送验证码；页面准确说明只有在后续个人 SMTP 自发自收验证成功后才能启用邮件提醒。
- [x] 认证失败使用统一中文错误，不泄露用户名或 QQ 邮箱是否存在；重复提交、网络失败和服务异常可恢复。
- [x] 会话令牌只通过安全 HttpOnly Cookie 传递，不出现在 JSON、URL、本地存储或日志；Cookie 的 SameSite、Secure、过期和撤销合同有测试。
- [x] 登录与注册页各自完整覆盖 loading、empty/初始、validation error、server error、authenticated/unauthenticated permission 和成功状态，不出现英文占位或“将在这里呈现”。
- [x] 已登录用户访问登录/注册页会按明确规则进入新聊天；未登录用户访问受保护路由只跳转一次到登录页。
- [x] 电脑端仅用键盘可以完成字段输入、密码显隐、错误定位、登录/注册互链和提交；标签、错误关联和焦点移动正确。
- [x] 页面使用 Issue 04 的 Claude-inspired BridGes 设计令牌，并在约定电脑端视口通过视觉回归。
...
2026-08-03 交付记录：契约改为 username + 严格 `\d+@qq.com` + identifier 双标识登录，AuthResponse 移除 session_token（仅 HttpOnly Cookie）；IdentityService 支持用户名规范化唯一、QQ 邮箱全局唯一与 change_username；登录/注册页按 Issue 04 设计令牌重写（BrandLogo、PasswordField 显隐、SMTP 验证说明、统一中文错误）；middleware 已登录访问登录/注册页进入新聊天（/），未登录单次跳转登录页；新增 NewChatHome 落地页。验证：pytest 1061 通过（含 cookie/QQ/用户名专项）、mypy/ruff（改动文件）、web typecheck/build、e2e chromium 65 通过（含 issue07 键盘路径、服务异常恢复、三视口视觉回归）。code-review 两轴审查完成并修复（契约 64/32 漂移、错误码误用、1440×900 视口缺口、Account.id 描述不符、服务异常恢复测试缺失）。
