...
- 2026-08-14：Issue 14 实现完成（分支 `issue-14-knowledge-ocr-real-qwen-audit`）。
  ...
  - **缓存与投影**：解析缓存命中 → 0 次 Qwen 调用、0 条新锁，投影标记 `parse_cache_hit` 并可引用产生文本的原始 run（`ocr_evidence_run_id`，迁移 47 新增列 + `document_parse_cache.ocr_run_id`；与 Issue 13 的迁移 46 并存）；缓存版本失效后的重新识别产生新 run 新锁，原锁不覆盖；摄取投影给出脱敏页数汇总（`ocr_pages_total/succeeded/failed`）。OCR 失败仍按既有合同诚实降级（"图片内容未做文字识别"），绝不标成已识别；recorder 写失败失败关闭（可重试），不形成"已 OCR"投影。
  ...
  - **验证**：Issue 建议回归命令通过（32 项）；ingestion 68 通过/5 项既有陈旧失败（main 同）；storage 71、architecture 10、ai 100、media 178、science 59、evaluation 80、retrieval 43+5 既有、runtime 43、chat 286 通过（比 main 基线少 1 失败）；迁移 47 幂等（合并时因 Issue 13 占用 46 而重编号）；mypy/ruff 对变更文件干净（残余为 main 既有）。全量 `tests` 结果见合并时复核（存在既有 collection 冲突与陈旧失败，与本分支基线一致）。
