...
- 2026-08-14：Issue 11 实现完成（分支 `issue-11-humanizer-real-qwen-audit`，commit ae0a4c1 + 复查修复 2c54b8f）。
  ...
  - 验证：`tests/humanizer/test_humanizer_model_run_locks.py`（30 用例：单锁/双锁顺序/未触发/停止/预算/首稿失败/修订失败/空输出/降级/结构校验失败/本地复核失败/幂等重放/恢复新锁/重启可查/跨账户隔离/recorder 写失败/缺锁/业务不匹配/脱敏/证据安全修订/旧路径）；`tests/architecture/test_humanizer_lock_seam.py`（全部 `gateway.invoke` 调用点必须经记录接缝 + 常量来源 + 主辅关联 + 稳定错误码，含负向样本）；真实 smoke `test_article_real_smoke.py` 增加重启后锁核对（锁数 == writing_call_count，首条必为首稿，固定模型 ID 来自单一事实源）。Issue 建议命令与 humanizer/architecture/ai/storage/credentials/evaluation/chat humanizer 相关套件全绿；`tests` 全量（排除 2 个同名文件冲突与 2 个本机常驻挂起的 runtime 测试后）3215 通过 / 211 失败，失败集与 pristine merge-base 逐项对比一致（唯一差异 openapi.json 漂移已通过重新生成修复并提交），全部为既有环境性失败（plugins/retirement/retrieval/ingestion/learning_projects 等，pristine main 相同文件集同样失败）。
  - 契约同步：`HumanizerResultProjection` 新增 `run_lock_id`/`model_run_id` 使 `openapi.json` 漂移，已用 `scripts/regenerate_openapi.py` 重新生成并提交（+24 行，仅新增两个字段 schema）；`generated.ts` 沿用仓库现状（Issue 01 后未再同步，测试只校验存在性）。
