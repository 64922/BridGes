    ...
    def _resolve_layers(
        ...
        *,
        ...
        if attachment_ids:
            ...
            if not layers[RetrievalSourceLayer.ATTACHMENT]["ready_document_ids"]:
        ...
        if project_id is not None:
            ...
            if not layers[RetrievalSourceLayer.PROJECT]["ready_document_ids"]:
                statuses = self._repository.document_statuses(
                    account_id, source="project_file", project_id=project_id
                ) | self._repository.document_statuses(
                    account_id, source="chat_attachment", project_id=project_id
                )
                status = RetrievalLayerStatus.NO_MATERIAL
                note = "项目暂无可检索文件。"
                if statuses & {"queued", "parsing", "processing"}:
                    status = RetrievalLayerStatus.INDEX_PROCESSING
                    note = "项目文件正在处理或建立索引。"
                elif "error" in statuses:
                    status = RetrievalLayerStatus.INDEX_CORRUPT
                    note = "项目文件索引损坏或处理失败，请重试。"
                layers[RetrievalSourceLayer.PROJECT].update(
                    status=status, note=note
                )
