...
规则（``docs/v2/workflows.md`` 第 3 节）：以 ``Asia/Shanghai`` **当前时刻**判断
距八个课间时间点（07:50、09:30、10:00、12:00、14:20、16:00、16:30、18:10）
是否在前后 **10 分钟**内；命中则提示「可能人多」，并在高德基础耗时之外增加
**5 分钟**。模块不知道用户何时出发，所以判定基准是提问时刻而不是抵达时刻。
