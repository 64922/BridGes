...
class VideoService:
    ...
    def process_pending(self) -> str:
        """执行一轮视频任务处理：逐账户领取 → 提交/轮询/取消收敛。"""
        accounts = [
            str(row["account_id"])
            for row in self._db.connection.execute(
                "SELECT DISTINCT account_id FROM video_tasks"
        ...
        return f"worker: 视频任务：本轮处理 {processed} 个任务步骤。"

    def _claim(self, account_id: str) -> list[str]:
        """领取一个可处理任务（单机 worker 每轮处理一个任务的一个步骤）。

        领取条件：queued（等待提交）、failed 且未超自动重试上限（自动
        恢复）、submitting/generating（云端轮询——每轮续期租约，进度由
        poll_count 与 MAX_CLOUD_POLLS 限制）、cancelling（收敛取消）。
        ``lease_expires_at`` 只用于呈现层恢复判定（进程中断期间未续期 →
        前端显示 recovery），不阻塞领取。取消与成功任务永不领取。
        """
        now = datetime.now(UTC)
        now_text = _now()
        ...
        condition = (
            "status IN ('queued', 'submitting', 'generating', 'cancelling')"
            f" OR (status = 'failed' AND retry_count < {MAX_AUTO_RETRIES}"
            # 永久失败（如供应商完成但结果缺失）不自动重领：重试不会因
            # 输入变化而成功，避免白耗生成配额；与前端隐藏重试入口一致。
        ...
        try:
            with self._db.transaction():
                cursor = self._db.scoped(account_id).execute(
                    ...
                    # 领取时把 queued/failed 提升为提交中；其余状态保持，
                    # 只续期租约。
                ...
                rows = self._db.scoped(account_id).execute(
                    ...
                    " AND status IN ('submitting', 'generating', 'cancelling')"
