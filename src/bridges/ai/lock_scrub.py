...
做关键词拒绝，供应商原文（如鉴权失败消息）常命中这些词导致锁无法
持久化。媒体边缘接缝（图片替代文本、图片/视频供应商取消）必须先脱敏
再落库：稳定错误码保留，正文一律不进入审计库。主生成链行为不变。
"""
...
from bridges.ai.sqlite_recorder import FREE_TEXT_FORBIDDEN_KEYWORDS
...
def scrub_lock_text(lock: ModelRunLock) -> ModelRunLock:
    ...
    关键词清单与录制器拒绝词单一事实源（sqlite_recorder 导出）。
    """
    ...
    for field in ("error_message", "degradation_reason"):
        ...
        if any(keyword in lowered for keyword in FREE_TEXT_FORBIDDEN_KEYWORDS):
