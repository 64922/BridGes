...
- [ ] 公共入口、认证主壳、导航、任务状态和上下文检查器使用同一设计令牌与可访问组件合同。
- [ ] 键盘焦点、路由公告、错误摘要、状态文本和跳转路径具有可重复的自动化测试。
- [ ] 关键布局在移动端、桌面端和 200% 文本缩放下不丢失操作、状态或内容关系。
- [ ] 动画、颜色、图标和实时更新均提供减少动画、冗余编码和可感知替代。
...
- [ ] 私人正文、私人索引、云端控制投影和项目副本具有明确且互不混淆的权威边界。
- [ ] 领域模块只依赖稳定保险库端口，不直接依赖设备框架、文件系统或云端供应商对象。
- [ ] 单次任务胶囊绑定主体、用途、对象、运行、授权版本、密钥时期和 TTL。
- [ ] 测试适配器、设备适配器和云端控制投影通过同一合同套件，设备不可用不会触发静默全文上传。
...
- [ ] 用户无法通过直接请求、深链、缓存键、任务引用或保险库对象引用读取另一用户内容。
- [ ] 后台任务缺少主体、对象域、授权版本或密钥时期时失败闭锁。
- [ ] 账户切换后不残留前一用户的页面、建议、通知或任务状态，并提供供后续存储与索引通道复用的隔离夹具。
...
- [ ] trace、metric、log 和审计事件使用统一运行、主体、项目与对象标识关联。
- [ ] 观测系统不复制私人正文、完整提示、密钥或不必要模型输出。
- [ ] SLI/SLO 注册表允许后续工作负载声明延迟、可用性、正确性和降级指标及责任人。
- [ ] 基础告警具有所有者、运行手册、去重和关闭证据，并提供后续票据复用的观测合同测试。
...
- [ ] 失效事件和墓碑不可变、版本化并绑定主体、对象域、原因、授权版本和密钥时期。
- [ ] 新读取和新运行先检查当前失效状态，缓存、索引和后台任务不能旁路。
- [ ] 影响集通过 Outbox 幂等传播到已登记下游，失败可重放且不会扩大作用域。
- [ ] 领域模块只能扩展影响解析和重验证处理器，不能改变墓碑优先、历史保留和失败闭锁下限。
...
- [ ] 来源状态变化通过通用失效基础生成不可变事件，并由科学领域解析器扩展影响集。
- [ ] 新任务和发布不会继续使用失效来源。
