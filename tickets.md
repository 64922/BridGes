...
- [x] 诊断报告定位具体文本、问题类型、理由和建议补丁。
- [x] 补丁应用前后自动比较事实锁、引用与措辞强度。
- [x] 用户反馈分别路由到当前版本、候选偏好、学习记录或事实复核。
- [x] AI 检测器分数不作为质量目标或通过门。
...
- [x] 版本比较突出事实锁、Claim、引用、结论强度和人工决定变化。
- [x] 工作流成功但产物未批准时发布入口保持锁定。
- [x] 来源、授权或上游事实锁失效会撤销当前发布资格。
- [x] 发布事件与实际版本、责任人和质量门结果绑定。
...
- [x] 图片区域、OCR、图例、尺度和置信度可查看与修正。
- [x] 公式保留符号表、结构、变量定义和可访问表示。
- [x] 表格保留 Schema、单位、缺失值、来源和原始数据对应。
- [x] 人工纠正创建派生版本并保留原始资产。
...
- [x] 音视频解析结果与原始时间轴和资产版本绑定。
- [x] 用户能修正术语、说话段、字幕与关键帧解释。
- [x] 多语言、低置信片段和缺失音轨被明确标记。
- [x] 私人媒体的中间结果遵守账户与项目作用域。
...
- [x] 图表数值、轴、单位、聚合和误差表达与源数据一致。
- [x] 科学图中的标签和关系绑定 Claim 与事实锁。
- [x] 用户能修改可编辑源并重新验证。
- [x] 每个视觉作品具备替代文本或等价数据表。
...
- [x] 分镜明确教学目标、对象、布局、状态、时间、旁白和 Claim 绑定。
- [x] 生成代码在无生产网络、无密钥和有限资源的隔离环境运行。
- [x] 失败只能在事实锁不变时有限修复，预算耗尽后成品隔离。
- [x] 沙箱记录依赖、资源、输出、错误和内容哈希。

## T034 — 生成朗读、字幕和完整无障碍替代

**What to build:** 让科学媒体对象拥有朗读、文字稿、字幕、替代文本、键盘路径、减少动画和顺序阅读版本。

**Blocked by:** T031 — 摄入并校正音频与视频材料；T032 — 生成可编辑静态科学图与数据图表；T033 — 生成结构化分镜并在沙箱运行交互或动画；T060 — 实现真实 Qwen ASR 适配器
...
- [x] 音频、字幕和文字稿共享同一科学 Claim 与版本。
- [x] 交互和媒体核心任务可由键盘与屏幕阅读器完成。
- [x] 用户可以暂停、控制时间内容并启用减少动画。
- [x] 无障碍替代经过相同科学校验，不形成另一个未经验证的答案。
...
- [x] 跨媒体核心事实、数值、术语、限定条件和引用一致。
- [x] 许可、真实性、沙箱和无障碍任一必需门失败会阻止发布。
- [x] 发布版本保留原始资产、分镜、可编辑源、渲染物和验证记录。
- [x] 来源或事实锁失效会传播到所有媒体派生版本。
...
- [x] 分享预览明确显示包含与排除字段、所有者和后续独立性。
- [x] 共享项目只引用项目对象，不挂载个人保险库正文。
- [x] 角色和对象授权共同决定查看、编辑、审阅与发布。
- [x] 邀请令牌短时、单用、绑定项目与预期身份。
...
- [x] 账户可加入多个互相不可见的机构。
- [x] 管理成员与读取内容是分开的权限和界面操作。
- [x] 机构自有项目创建前披露所有权、保留和恢复边界。
- [x] 安全事件正文访问需要最小范围、双人批准、时限和完整审计。
...
- [x] 设备证书、密钥时期和配对状态与用户账户绑定。
- [x] 设备私钥进入系统密钥库，不写入普通配置或日志。
- [x] 个人正文和私人索引默认在设备侧保持明文权威。
- [x] 云端只能获得与单次运行、用途、对象和 TTL 绑定的任务胶囊。
...
- [x] 同步先拉取策略、撤权、密钥时期和墓碑，再提交本地操作。
- [x] 旧密钥时期或已撤权设备的提交被拒绝并隔离。
- [x] 删除墓碑优先于离线编辑，恢复后对象不会复活。
- [x] 科学判断、事实锁、归属和授权冲突不使用最后写入者获胜。
...
- [x] Manifest 声明范围、排除项、来源、规则、能力、夹具、依赖和风险。
- [x] 加载器验证 Schema、摘要、依赖锁、能力合同和兼容范围，并通过生产工作流运行候选夹具。
- [x] 领域规则不能放宽平台权限、安全、审计和发布下限。
- [x] 包升级创建新版本并提供兼容、迁移、失效和受信回滚信息。
...
- [x] 数学包声明适用范围、排除项、权威来源、术语和版本兼容范围。
- [x] 定义、符号、前提、推理步骤和证明状态具有确定性规则或明确人工门。
- [x] 夹具覆盖正确证明、缺失前提、循环论证、符号冲突和无法判定状态。
- [x] 包输出保持 Claim、Evidence、事实锁和验证报告可追溯，不能把模型生成步骤直接视为已证明。
...
- [x] 包定义学科来源层级、术语、单位和证据映射。
- [x] 测量、实验条件、误差和化学规则有确定性夹具。
- [x] 危险实验建议进入安全拒绝或人工门。
- [x] 包与公共平台规则组合后保持 Claim 和措辞可追溯。
...
- [x] 生命科学与医学包分别声明适用和排除范围。
- [x] 医学高风险内容不能形成诊断、处方或替代专业决策。
- [x] 关键来源状态未知或证据冲突时高置信发布闭锁。
- [x] H3 变更要求合资格领域专家和安全治理联合确认。
...
- [x] 包能区分观测事实、模型输出、情景和推断。
- [x] 时间、空间、仪器和数据版本进入 Claim 限定条件。
- [x] 数据修订和来源状态变化能触发影响分析。
- [x] 夹具覆盖尺度误用、相关因果混淆和模型外推。
...
- [x] 软件与规范结论绑定明确版本、发布日期和状态。
- [x] 数据集来源、许可、切片、更新和撤回信息可追溯。
- [x] 过时文档、废弃 API 和非权威博客不会覆盖有效一手规范。
- [x] 跨学科规则冲突按明确优先级进入人工门。
...
- [x] 内容签名、独立验证签名和平台发行签名绑定同一规范化摘要。
- [x] 同一自然人不能对同一版本同时充当维护者和独立复核者。
- [x] 语义 Diff 显示规则、措辞、人工门、来源和夹具的判定变化。
- [x] 灰度只生成可发行候选，不能自动激活。
...
- [x] 失效事件按检测、分诊、控制、定位影响、修复、重验证和关闭推进。
- [x] 安全管理员可紧急撤销，但不能编辑规则或直接发布替代版本。
- [x] 影响集覆盖包、运行、Claim、Evidence、Wording、产物、项目和用户动作。
- [x] 回滚不能复活已撤销、签名无效或不兼容版本。
...
- [x] 支持 `qwen3-asr-flash` 和 `qwen3-asr-flash-filetrans` 的官方输入限制（时长、大小、格式）。
- [x] 返回结果绑定原始资产时间轴、版本和账户/项目作用域。
- [x] 低置信术语、多语言片段和缺失音轨被明确标记。
- [x] ASR 失败时保留原始资产，产物进入可人工校正状态，不标记为完成。
...
- [x] 支持 `qwen-vl-ocr` 及视觉主模型的输入限制和区域可用性探测。
- [x] 模型输出作为候选派生资产，低置信区域明确标记，不直接进入事实锁。
- [x] 坐标/页码/时间定位绑定原始资产版本，校正后产生新版本。
- [x] 失败时保留原始资产，不伪造数据点或文本。
...
- [x] 只接受已通过事实锁的文本输入，不直接为未经验证 claim 生成语音。
- [x] 数字、单位、公式、缩写和外文专名有发音处理或显式降级提示。
- [x] 供应商临时 URL 在有效期内转存，记录 MIME、大小、哈希和归属。
- [x] TTS 失败时文本回答仍可交付，媒体对象标记合成失败。
